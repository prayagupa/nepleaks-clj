...
(ns nepleaks-engine.services.leaksSplitterTridentClient
  ...
  (:require [nepleaks-engine.services.leaksSplitterTridentFunction]
...
(defn mk-drpc-stream
  ...
  (-> (.newDRPCStream topology "words" drpc)
      (.each (Fields. ["args"]) (org.nepleaksengine.services.LeakSplitter.) (Fields. ["word"]))
      (.groupBy (Fields. ["word"]))
...
(defn mk-wordcount-topology
  ...
  (-> topology
      ...
      (.each (Fields. ["str"])
             (org.nepleaksengine.services.LeakSplitter.)
             (Fields. ["word"]))
      (.groupBy (Fields. ["word"]))
